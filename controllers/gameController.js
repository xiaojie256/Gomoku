...
exports.submitMove = async (req, res) => {
  const { boardId, userId, x, y, player } = req.body;
  ...
  try {
    ...
    if (daysDiff > 10 && board.status === "playing") {
        await client.query("UPDATE boards SET status = 'draw' WHERE id = $1", [boardId]);
    ...
    if (player !== expectedPlayer) throw new Error(`当前该${expectedPlayer === 1 ? "黑子" : "白子"落子`);
    ...
    currentMoves.forEach((move) => { currentBoard[move.y * 15 + move.x] = move.player; });
    
    const positionIndex = y * 15 + x;
    if (currentBoard[positionIndex] !== 0) throw new Error("该位置已被占用");

    pointer = wasmModule._malloc(900);
    for (let i = 0; i < 225; i++) { wasmModule.setValue(pointer + i * 4, currentBoard[i], "i32"); }
...
exports.createGame = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, boardId: result.rows[0].id, secretCode: result.rows[0].secret_code });
  } catch (err) {
...
exports.verifySecretCode = async (req, res) => {
  ...
  const client = await pool.connect();
  try {
    // 归一化输入，去除前后空白并转为大写，避免用户输入大小写或空格导致匹配失败
...
exports.resetGame = async (req, res) => {
  ...
  if (!boardId || !userId) return res.status(400).json({ error: '缺少参数' });
...
exports.endGameInAdvance = async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length === 0) return res.status(403).json({ error: "仅限房主操作或棋局已结束" });
        res.json({ success: true });
    } catch (err) {
...
exports.getUserHistory = async (req, res) => {
    ...
    try {
        const result = await client.query(
            "SELECT id, status, is_public FROM boards WHERE black_user_id = $1 OR white_user_id = $1 ORDER BY created_at DESC LIMIT 30",
            [userId]
        );
...
exports.getGame = async (req, res) => {
  ...
  try {
    const boardRes = await client.query(
      "SELECT id, status, winner_id, black_user_id FROM boards WHERE id = $1",
      [boardId],
    );
    if (boardRes.rows.length === 0) {
